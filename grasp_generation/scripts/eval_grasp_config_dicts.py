...
def compute_joint_angle_targets(
    ...
    grasp_orientations_array = torch.stack(grasp_orientations_array, dim=0).to(device)
    ...
    for grasp_config_dict_path in pbar:
        ...
        for i in range(batch_size):
            passed_simulation[i] = np.array(
                sum(all_successes[i * num_envs_per_grasp : (i + 1) * num_envs_per_grasp])
                == num_envs_per_grasp
            )
