...
# %%
existing_meshdata_path = pathlib.Path("/juno/u/tylerlum/github_repos/DexGraspNet/data/rotated_meshdata_v2_only")
assert existing_meshdata_path.exists()

# %%
existing_meshdata_folders = sorted(list(existing_meshdata_path.iterdir()))
assert len(existing_meshdata_folders) > 1500

# %%
num_per_folder = 250
N_new_folders = math.ceil(len(existing_meshdata_folders) / num_per_folder)
new_meshdata_folders = [
    existing_meshdata_path.parent / f"2024-05-26_{existing_meshdata_path.name}_{i}"
