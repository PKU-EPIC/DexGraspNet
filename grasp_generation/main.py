...
energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_tpen = cal_energy(hand_model, object_model, verbose=True, **weight_dict)

energy.sum().backward(retain_graph=True)
logger.log(energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_tpen, 0, show=False)

for step in tqdm(range(1, args.n_iter + 1), desc='optimizing'):
    s = optimizer.try_step()

    optimizer.zero_grad()
    new_energy, new_E_fc, new_E_dis, new_E_pen, new_E_spen, new_E_joints, new_E_tpen = cal_energy(hand_model, object_model, verbose=True, **weight_dict)
    ...
    with torch.no_grad():
        ...
        logger.log(energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_tpen, step, show=False)
