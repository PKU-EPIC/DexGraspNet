...
# %%
print([i for i in range(len(preds)) if 1.0 > preds[i] > 0.7])
...
for i in tqdm(range(10000)):
    ...
    point_list.append(point_torch.tolist()[:6])
    grad_list.append(point_torch.grad.tolist()[:6])
    with torch.no_grad():
        # step_size = 0.000001
        step_size = 0.00001
        # if point_torch.grad[:6].norm() <= 1e-1:
        #     print("SMALL")
        #     print(f"point_torch.grad[:6].norm() = {point_torch.grad[:6].norm()}")
        #     step_size = 0.1 / point_torch.grad[:6].norm()
        # else:
        #     print(f"point_torch.grad[:6].norm() = {point_torch.grad[:6].norm()}")
        # print(f"point_torch.grad[:6].norm() = {point_torch.grad[:6].norm()}")
        update_step = point_torch.grad[:6] * step_size
        max_magnitude = 0.001
        ...
        point_torch[:6] = point_torch[:6] + update_step
    if pred.item() > 0.9:
        consecutive_high += 1
    else:
        consecutive_high = 0
...
# %%
plt.plot(np.linalg.norm(grad_arr[:, 0:6], axis=1))
plt.title("grad norm")
