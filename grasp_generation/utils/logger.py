...
class Logger:
    ...
    def log(self, energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_tpen, step, show=False):
        ...
        if show:
            print(f'Step %d energy: %f  fc: %f  dis: %f  pen: %f  spen: %f  joints: %f  tpen: %f' % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean(), E_spen.mean(), E_joints.mean(), E_tpen.mean()))
